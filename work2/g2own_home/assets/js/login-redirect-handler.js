...
class LoginRedirectHandler {
    constructor() {
      ...
      async verifySessionAfterLogin() {
        ...
        if (window.sessionBridge && typeof window.sessionBridge.checkSession === 'function') {
            try {
                // Use enhanced session check endpoint
                const forceCheckUrl = `${window.sessionBridge.baseURL}/enhanced-session-check.php?force=1&_=${Date.now()}`;
                console.log('🔍 Forcing session check with:', forceCheckUrl);
                
                const requestOptions = window.securityUtils 
                ? window.securityUtils.secureRequest(forceCheckUrl)
                : {
                    method: 'GET',
                    credentials: 'include',
                    headers: {
                ...
                const response = await fetch(forceCheckUrl, requestOptions);
                
                if (response.ok) {
                    const data = await response.json();
                    console.log('✅ Enhanced session check response:', data);
                    ...
                    if (data.authenticated && data.user) {
                        ...
                        if (window.sessionBridge) {
                            window.sessionBridge.currentUser = data.user;
                            window.sessionBridge.isAuthenticated = true;
                            window.sessionBridge.lastUserId = data.user.id;
                            
                            // Update main site UI
                            window.sessionBridge.updateMainSiteAuth(data.user);
                        }
                          // Store user data in safe storage for backup
                        if (window.safeStorage) {
                            window.safeStorage.setItem('g2own_user', JSON.stringify(data.user));
                        } else {
                            try {
                                localStorage.setItem('g2own_user', JSON.stringify(data.user));
                            } catch(e) {
                                console.warn('⚠️ Could not store user data in localStorage');
                                // Try sessionStorage as fallback
                                try {
                                    sessionStorage.setItem('g2own_user', JSON.stringify(data.user));
                                } catch(e2) {
                                    console.warn('⚠️ Could not store user data in sessionStorage');
                                }
                            }
                        }
                        
                        // Show success notification
                        this.showLoginSuccessNotification(data.user);
    ...
    isLoginFlowActive() {
        ...
        if (window.safeStorage) {
            window.safeStorage.removeItem('g2own_login_initiated');
            window.safeStorage.removeItem('last_login_redirect');
        } else {
            sessionStorage.removeItem('g2own_login_initiated');
            sessionStorage.removeItem('last_login_redirect');
            try {
                localStorage.removeItem('g2own_login_initiated');
                localStorage.removeItem('last_login_redirect');
            } catch(e) {
                console.warn('⚠️ Could not remove from localStorage');
            }
        }
        ...
        setTimeout(() => {
            ...
            // Use new session sync method if available
            if (window.syncCommunitySession) {
                console.log('🔄 Using new session sync after login success');
