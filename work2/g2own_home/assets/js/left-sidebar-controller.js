...
class LeftSidebarController {
    ...
    setupAuthHandlers() {
        ...
        console.log('🔑 Left sidebar login/signup clicked - checking session first');
            ...
            try {
                ...
                if (isAlreadyAuthenticated) {
                    ...
                    if (this.sessionBridge.currentUser) {
                        this.updateAuthUI(this.sessionBridge.currentUser);
                    }
        ...
        if (this.sessionBridge && typeof this.sessionBridge.redirectToLogin === 'function') {
            this.sessionBridge.redirectToLogin();        } else {
            // Demo mode - no real redirects
            const returnUrl = encodeURIComponent(window.location.href);
            console.log('📍 Demo mode - no real redirects:', `#/community/login/?return=${returnUrl}`);
            // Demo mode - no real redirects
        }
    }
    ...
    handleFooterLink(href) {
      ...
      handleSocialAuth(provider) {
        ...
        // Close sidebar first
        this.close();
          // Demo mode - no real social auth
        const returnUrl = encodeURIComponent(window.location.href);
        
        switch (provider) {
            case 'google':
                console.log('Demo mode - Google login disabled');
                break;
            case 'discord':
                console.log('Demo mode - Discord login disabled');
                break;
            case 'steam':
                console.log('Demo mode - Steam login disabled');
                break;
            default:
    ...
    forceUpdate(userData = null) {
        console.log('🔄 Force updating left sidebar auth state...', userData?.name);
