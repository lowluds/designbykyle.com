...
class ProductDisplayOAuth {
    ...
    async loadProducts() {
      ...
      /**
     * Load user-specific data (purchases, favorites, etc.) using appropriate API keys
     */
    async loadUserData() {
        if (!this.oauth.isUserAuthenticated()) {
            return;
        ...
        try {
            ...
            // Load user purchases using menus API key
            await this.loadUserPurchases();
            
            // Load user favorites using authorized_user API key
    ...
    async loadUserFavorites() {
        try {
            const response = await this.oauth.makeAuthenticatedRequest(
                ...
                { method: 'GET' },
                'authorizedUser' // Use authorized_user API key for user operations
            );
      ...
      /**
     * Toggle product favorite using authorized_user API key
     */
