...
class OAuthTesting {
    ...
    getTestPanelHTML() {
        return `
            ...
            <div class="test-panel-content" id="test-panel-content">
                ...
                <div class="test-section">
                    <h4>Notification Testing</h4>
                    <div class="test-actions">
                        <div class="test-input-group">
                            <button id="test-check-notifications" class="test-btn">Check Notifications</button>
                            <button id="test-toggle-notification-dot" class="test-btn">Toggle Dot</button>
                            <button id="test-simulate-notification" class="test-btn">Simulate Notification</button>
                        </div>
                        <div class="test-display">
                            <span>Notification Count: <span id="notification-count-display">0</span></span>
                        </div>
                    </div>
    ...
    setupEventListeners() {
        ...
        document.getElementById('test-check-notifications').addEventListener('click', () => {
            this.testCheckNotifications();
        });
        
        document.getElementById('test-toggle-notification-dot').addEventListener('click', () => {
            this.testToggleNotificationDot();
        });
        
        document.getElementById('test-simulate-notification').addEventListener('click', () => {
            this.testSimulateNotification();
        });
    ...
    async testCheckNotifications() {
        try {
            this.log('Checking notifications...');
            
            if (window.oauthNotifications) {
                const result = await window.oauthNotifications.fetchNotifications();
                this.log('Notifications result:', result);
                
                // Update display
                const countDisplay = document.getElementById('notification-count-display');
                if (countDisplay) {
                    countDisplay.textContent = result.unread_count || 0;
                }
                
                this.updateDisplay('check-notifications', result);
            } else {
                this.log('OAuth Notifications not available', 'error');
            }
        } catch (error) {
            this.log('Error checking notifications:', error, 'error');
        }
    }
    
    testToggleNotificationDot() {
        try {
            const dot = document.querySelector('.notification-dot');
            if (dot) {
                const currentDisplay = window.getComputedStyle(dot).display;
                const newDisplay = currentDisplay === 'none' ? 'block' : 'none';
                dot.style.display = newDisplay;
                ...
                this.log(`Notification dot ${newDisplay === 'block' ? 'shown' : 'hidden'}`);
            } else {
                this.log('Notification dot not found', 'error');
            }
        } catch (error) {
            this.log('Error toggling notification dot:', error, 'error');
        }
    }
    
    testSimulateNotification() {
        try {
            // Simulate a notification by updating the count
            const randomCount = Math.floor(Math.random() * 5) + 1;
            
            if (window.leftSidebarController) {
                window.leftSidebarController.updateNotificationCount(randomCount);
                this.log(`Simulated ${randomCount} notifications`);
                
                // Update display
                const countDisplay = document.getElementById('notification-count-display');
                if (countDisplay) {
                    countDisplay.textContent = randomCount;
                }
            } else {
                this.log('Left Sidebar Controller not available', 'error');
            }
        } catch (error) {
            this.log('Error simulating notification:', error, 'error');
        }
    }
}
