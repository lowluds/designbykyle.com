...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initializeTopSearch() {
        ...
        let isSearchOpen = false;        // Toggle search dropdown visibility
        ...
        searchToggle.addEventListener('mousedown', function(e) {
            console.log('🖱️ Search button mousedown event triggered');
        });
        
        searchToggle.addEventListener('mouseup', function(e) {
            console.log('🖱️ Search button mouseup event triggered');
        });

        console.log('✅ Search toggle event listeners attached successfully');

        // Force event listener attachment (backup approach)
        setTimeout(() => {
            console.log('🔧 Applying backup event listener...');
            const backupToggle = document.getElementById('top-search-toggle');
            if (backupToggle) {
                // Remove any existing listeners and add our own
                const newButton = backupToggle.cloneNode(true);
                backupToggle.parentNode.replaceChild(newButton, backupToggle);
                ...
                newButton.addEventListener('click', function(e) {
                    ...
                    if (dropdown && input) {
                        if (dropdown.style.display === 'none' || !dropdown.classList.contains('active')) {
                            dropdown.style.display = 'block';
                            setTimeout(() => {
                                dropdown.classList.add('active');
                                input.focus();
                                console.log('🎯 Search dropdown opened via backup handler');
                            }, 10);
                        } else {
                            dropdown.classList.remove('active');
                            setTimeout(() => {
                                dropdown.style.display = 'none';
                                console.log('🎯 Search dropdown closed via backup handler');
                            }, 300);
                        }
                    }
                });
                
                console.log('🔧 Backup event listener attached successfully');
            }
        }, 500);// Clear search button
        if (searchClear) {
