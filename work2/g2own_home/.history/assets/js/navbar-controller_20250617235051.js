...
class NavbarController {    constructor() {
        ...
        this.loginSignupBtn = document.getElementById('loginSignupBtn') || document.getElementById('oauth-login-btn');
        ...
        this.initializeOAuthIntegration();
    ...
    setupAuthenticationHandlers() {
        ...
        console.log('🔑 Login/Signup button clicked from navbar - redirecting to community');
        
        if (this.authBridge && this.authBridge.redirectToLogin) {
            this.authBridge.redirectToLogin();
        } else {
            // Use the same approach as other auth systems
    ...
    setupEventListeners() {
        ...
        // Search functionality - DISABLED (handled by search.js)
        ...
        // Search input functionality - DISABLED (handled by search.js)
        // if (this.searchInput) {
      ...
      setupNavLinks() {
        ...
        navLinks.forEach(link => {
            link.addEventListener('click', (e) => {
                ...
                // For external links (like support), let the browser handle them normally
    ...
    openMobileMenu() {
      ...
      setupCartFunctionality() {        const cartToggle = document.getElementById('cart-toggle');
    ...
    async initializeOAuthIntegration() {
        // Wait for OAuth manager to be ready
        if (window.g2ownOAuth) {
            this.oauthManager = window.g2ownOAuth;
            await this.oauthManager.init();
            this.oauthCart = this.oauthManager.cart;
            
            // Set up OAuth event listeners
            this.setupOAuthEventListeners();
            
            // Initial cart sync
            this.syncCartWithOAuth();
            
            console.log('✅ Navbar OAuth integration initialized');
        } else {
            // Retry after a delay
            setTimeout(() => this.initializeOAuthIntegration(), 500);
        }
    }
    
    setupOAuthEventListeners() {
        // Listen for cart updates from OAuth system
    ...
    syncCartWithOAuth() {
        if (this.oauthCart) {
            const cart = this.oauthCart.getCart();
            this.updateCartCount(cart.count || 0);
            this.state.cartItems = cart.count || 0;
        }
    }
    
    handleCheckout() {
        if (this.oauthCart && this.oauthManager) {
            // Check if user is authenticated
            if (this.oauthManager.oauth && this.oauthManager.oauth.isUserAuthenticated()) {
                // User is logged in, proceed to checkout
                this.oauthCart.initiateCheckout();
            } else {
                // User needs to log in first
                this.showCheckoutLoginPrompt();
            }
        } else {
            // Fallback if OAuth cart not available
            this.showCheckoutLoginPrompt();
        }
    }
    
    showCheckoutLoginPrompt() {
        const message = 'Please sign in to continue with checkout';
    ...
    // ===== TEST METHODS FOR CART FUNCTIONALITY =====
    testAddToCart() {
        if (this.oauthCart) {
            // Test adding a mock product
            this.oauthCart.addToCart('test-product-1', 1);
            console.log('Added test product to cart');
        } else {
            // Fallback test - just update the UI
            const currentCount = this.state.cartItems;
            this.updateCartCount(currentCount + 1);
            console.log('Added test item to cart (UI only)');
        }
    }
    
    testRemoveFromCart() {
        if (this.oauthCart) {
            this.oauthCart.removeFromCart('test-product-1');
            console.log('Removed test product from cart');
        } else {
            // Fallback test
            const currentCount = Math.max(0, this.state.cartItems - 1);
            this.updateCartCount(currentCount);
            console.log('Removed test item from cart (UI only)');
        }
    }
    
    testCheckOAuthConnection() {
        console.log('OAuth Manager:', this.oauthManager);
        console.log('OAuth Cart:', this.oauthCart);
        console.log('Is OAuth Available:', !!window.g2ownOAuth);
        
        if (this.oauthManager && this.oauthManager.oauth) {
            console.log('Is User Authenticated:', this.oauthManager.oauth.isUserAuthenticated());
        }
    }
}
