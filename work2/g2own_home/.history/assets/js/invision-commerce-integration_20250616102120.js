...
(function() {
    ...
    function addCartIndicator() {
        // Find a safe place to add cart indicator
        const navLinks = document.querySelector('.nav-links');
        const userActions = document.querySelector('.top-nav-user-actions');
        const targetContainer = navLinks || userActions;
        
        if (targetContainer && !document.querySelector('.community-cart-link')) {
            const cartLink = document.createElement('a');
            cartLink.href = STORE_URL + 'cart/';
            cartLink.target = '_blank';
            cartLink.className = 'community-cart-link';
            cartLink.title = 'View Shopping Cart';
            ...
            cartLink.innerHTML = '<i class="ph ph-shopping-cart"></i>';
            
            // Add hover effect
            cartLink.addEventListener('mouseenter', function() {
                this.style.background = 'rgba(139, 0, 0, 0.2)';
            ...
            targetContainer.appendChild(cartLink);
        }
    }
