...
class G2OwnOAuth {
    ...
    buildAuthorizationUrl() {
        ...
        // Check if we're returning from OAuth success with proper validation
        const hasLoginSuccess = urlParams.get('login') === 'success' || urlParams.get('status') === 'success';
        const hasCode = urlParams.has('code');
        const hasToken = urlParams.has('token');
        
        if (hasLoginSuccess && (hasCode || hasToken)) {
            console.log('✅ OAuth return detected with proper parameters!');
            this.handleOAuthReturn(urlParams);
        } else if (hasLoginSuccess) {
            console.log('⚠️ OAuth login success parameter found but no code/token - verifying session');
    ...
    async verifySessionAfterLogin() {
        console.log('🔍 Verifying session after OAuth login redirect...');
        ...
        if (window.sessionBridge && typeof window.sessionBridge.checkSession === 'function') {
            try {
                ...
                if (window.sessionBridge.isAuthenticated && window.sessionBridge.currentUser) {
                    console.log('✅ Valid session found after OAuth login redirect');
                    
                    // Create synthetic OAuth data from session user
                    const syntheticOAuthData = {
                        token: 'session_import_' + Date.now(),
                        user: window.sessionBridge.currentUser,
                        source: 'session_import'
                    };
                    
                    // Process as if it were a normal OAuth return
                    await this.storeOAuthData(syntheticOAuthData);
                    this.cleanUpUrl();
                    await this.triggerAuthUpdate(syntheticOAuthData);
                    
                    console.log('🎉 Session imported successfully without redirect!');
                } else {
                    console.warn('⚠️ No valid session found after OAuth login redirect');
                    this.cleanUpUrl();
                }
            } catch (error) {
                console.error('❌ Error verifying session after OAuth login:', error);
                this.cleanUpUrl();
            }
        } else {
    ...
    async handleOAuthReturn(urlParams) {
        try {
            ...
            if (oauthData && oauthData.token) {
                ...
                console.log('🎉 OAuth authentication completed successfully!');            } else {
                // No valid token found, verify session as fallback
                this.verifySessionAfterLogin();
            }
        } catch (error) {            console.error('❌ OAuth return handling failed:', error);
