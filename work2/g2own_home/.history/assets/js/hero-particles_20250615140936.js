...
class G2OwnHeroParticles {
    ...
    updateParticles() {
        this.particles.forEach(particle => {
            ...
            particle.y += particle.vy;
              // Update life for enhanced pulsing
            particle.life += 0.02;
            particle.opacity = (Math.sin(particle.life) * 0.3 + 0.6) * 0.8; // Enhanced pulsing visibility
    ...
    drawParticles() {
        ...
        // Enhanced connection colors - mix of red and white
        const connectionColors = [
            'rgba(239, 68, 68, 0.3)',   // Red connections
            'rgba(255, 255, 255, 0.2)', // White connections
            'rgba(220, 38, 38, 0.25)',  // Deep red connections
            'rgba(248, 113, 113, 0.2)'  // Light red connections
        ];
        ...
        for (let i = 0; i < this.particles.length; i++) {
            for (let j = i + 1; j < this.particles.length; j++) {
                ...
                if (distance < this.config.connectionDistance) {
                    ...
                    // Choose connection color based on connected particles
                    const colorIndex = (i + j) % connectionColors.length;
                    this.ctx.strokeStyle = connectionColors[colorIndex];
