<?php
/**
 * Enhanced G2Own Session Check Endpoint
 * Cross-domain authentication bridge for Invision Community
 * 
 * This file provides a more robust session check with additional protections
...
function forceCheckSession() {
    ...
    // Check both cookies and session
    $member_id = null;
    ...
    // Check cookies if session check failed
    if (!$member_id && isset($_COOKIE['member_id']) && $_COOKIE['member_id'] > 0) {
        $member_id = $_COOKIE['member_id'];
    }
    
    // If we found a member ID, load the member
    if ($member_id) {
        ...
        if ($member->member_id) {
            ...
            // Log the forced session check for debugging
            \IPS\Log::debug("Forced session check successful for user: " . $member->name, 'auth_bridge');
        }
    }
