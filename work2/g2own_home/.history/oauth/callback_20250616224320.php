...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $tokenData = exchangeCodeForToken($code, $config);            // Fetch user data
            $userData = fetchUserData($tokenData['access_token'], $config);
            
            // Fetch commerce data
            $commerceData = fetchUserCommerceData($tokenData['access_token'], $config);
            
            // Return both token, user and commerce data
            $response = array_merge($tokenData, [
                'user' => $userData,
    ...
    // Handle GET request for direct callback (browser redirect)
        ...
        // Merge user and commerce data
        $enhancedUserData = array_merge($userData, $commerceData);
        ...
        $_SESSION['oauth_user'] = $enhancedUserData;
        $_SESSION['oauth_expires'] = time() + ($tokenData['expires_in'] ?? 3600);
        $_SESSION['api_keys'] = $config['api_keys']; // Store API keys for frontend use
