...
$config = [
    'client_id' => '86490564787ccfdb2fefcaed4b49138f', // G2Own Website Integration Client ID
    'client_secret' => '', // Not needed for frontend-only flow
    ...
    'api_keys' => [
        ...
        'email' => 'email', // For email access
        'profile' => 'profile' // For profile access
    ]
];
    ...
    private function makeCurlRequest($url, $postData = null, $headers = [], $method = 'GET') {
      ...
      private function redirectWithSuccess($tokenData, $userData) {
        ...
        if (isset($this->config['api_keys'])) {
            $_SESSION['api_keys'] = $this->config['api_keys'];
        }
        
        // Redirect to main site with login success parameter
        $mainSiteUrl = $this->config['frontend_url'] . '?' . http_build_query([
            'login' => 'success',
