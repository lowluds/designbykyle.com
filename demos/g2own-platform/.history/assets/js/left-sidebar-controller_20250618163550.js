...
class LeftSidebarController {
    ...
    updateProfileDisplay() {
        ...
        if (avatar) {
            avatar.src = this.currentUser.avatar || 'assets/images/default-avatar.svg';
            avatar.alt = `${this.currentUser.name}'s avatar`;
        }
        if (name) name.textContent = this.currentUser.name || this.currentUser.username;
        if (level) level.textContent = `Level ${this.currentUser.level || 1} Trader`;
        
        if (stats.length >= 2) {
            stats[0].textContent = `⭐ ${this.currentUser.rating || 0}`;
            stats[1].textContent = `🛒 ${this.currentUser.sales || 0} Sales`;
        }
    }
      checkAuthState() {
    ...
    handleSocialAuth(provider) {
        ...
        // Redirect to community social auth
        const returnUrl = encodeURIComponent(window.location.href);
        
        switch (provider) {
            case 'google':
                window.location.href = `https://g2own.com/community/login/google/?ref=${returnUrl}`;
                break;
            case 'discord':
                window.location.href = `https://g2own.com/community/login/discord/?ref=${returnUrl}`;
                break;
            case 'steam':
                window.location.href = `https://g2own.com/community/login/steam/?ref=${returnUrl}`;
                break;
            default:
