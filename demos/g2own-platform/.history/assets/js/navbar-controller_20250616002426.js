...
class NavbarController {
    ...
    init() {
        ...
        this.setupNavbarAnimations(); // Add animation setup
    ...
    toggleCart() {
        ...
        // Emit cart toggle event for other components
    ...
    setupNavbarAnimations() {
        // Trigger navbar loaded state after initial delay
        setTimeout(() => {
            this.navbar.classList.remove('navbar-loading');
            this.navbar.classList.add('navbar-loaded');
        }, 200);
        ...
        // Setup icon interaction animations
        this.setupIconAnimations();
        
        // Setup logo pulse animation
        this.setupLogoPulse();
    ...
    setupScrollAnimations() {
        ...
        const updateScrollAnimations = () => {
            ...
            // Animate navbar items based on scroll direction
            const icons = document.querySelectorAll('.nav-gaming-icon');
            if (scrollY > lastScrollY && scrollY > 100) {
                // Scrolling down - subtle scale down
                icons.forEach(icon => {
                    icon.style.transform = 'scale(0.9)';
                });
            } else {
                // Scrolling up - restore scale
                icons.forEach(icon => {
    ...
    setupIconAnimations() {
        ...
        navLinks.forEach((link, index) => {
            ...
            link.addEventListener('mouseenter', () => {
                ...
                this.triggerIconEffect(icon, link.dataset.section);
            });
        ...
        this.setupActionButtonAnimations();
    }
    ...
    setupActionButtonAnimations() {
        ...
        if (this.searchToggle) {
            this.searchToggle.addEventListener('click', () => {
                const icon = this.searchToggle.querySelector('.nav-gaming-icon');
                this.triggerSearchAnimation(icon);
            });
        }
        ...
        if (this.cartToggle) {
            this.cartToggle.addEventListener('click', () => {
                const icon = this.cartToggle.querySelector('.nav-gaming-icon');
                this.triggerCartAnimation(icon);
            });
        }
    }
    
    triggerIconEffect(icon, section) {
        // Apply section-specific effects using existing animations
        switch(section) {
            case 'marketplace':
                icon.style.animation = 'bounceScale 0.6s ease-in-out';
                break;
            case 'games':
                icon.style.animation = 'controller-pulse 0.8s ease-in-out';
                break;
            case 'digital-goods':
                icon.style.animation = 'particleFloat 1s ease-in-out';
                break;
            case 'support':
                icon.style.animation = 'pulseGlow 0.8s ease-in-out';
                break;
        }
    ...
    triggerClickAnimation(icon) {
        icon.style.animation = 'scaleInBounce 0.4s ease-out';
    ...
    triggerSearchAnimation(icon) {
        if (this.state.isSearchOpen) {
            icon.style.animation = 'rotateIn 0.5s ease-out';
        } else {
            icon.style.animation = 'scaleInBounce 0.5s ease-out';
        }
    ...
    triggerCartAnimation(icon) {
        icon.style.animation = 'cart-shake 0.5s ease-in-out';
    ...
    setupLogoPulse() {
        ...
        if (logoIcon) {
            // Add continuous subtle pulse every 5 seconds
            setInterval(() => {
                logoIcon.style.animation = 'glow 1s ease-in-out';
    ...
    setupSearchAnimations() {
        if (this.searchDropdown) {
            // Observer for search dropdown visibility
            const observer = new MutationObserver((mutations) => {
                mutations.forEach((mutation) => {
                    if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
                        if (this.searchDropdown.classList.contains('active')) {
                            this.searchDropdown.style.animation = 'slideInDown 0.3s ease-out';
                        }
                    }
                });
            });
            
            observer.observe(this.searchDropdown, {
                attributes: true,
    ...
    animateElement(element, animationName, duration = '0.5s') {
    ...
    animateCartCount() {
        if (this.cartCount) {
            this.cartCount.style.animation = 'scaleInBounce 0.5s ease-out';
            
            setTimeout(() => {
                this.cartCount.style.animation = 'float 2s ease-in-out infinite';
            }, 500);
        }
    }
    
    // Override existing setCartCount to include animation
    setCartCount(count) {
        if (this.cartCount) {
