    ...
    updateAuthState(user = null) {
      ...
      updateProfileDisplay() {
        ...
        if (avatar) {
            ...
            avatar.alt = `${this.currentUser.name || this.currentUser.display_name || 'User'}'s avatar`;
        ...
        // Update name
        if (name) {
            name.textContent = this.currentUser.name || this.currentUser.display_name || this.currentUser.username || 'User';
        }
        
        // Update level/title
        if (level) {
            const userLevel = this.currentUser.level || this.currentUser.group?.name || 'Member';
            level.textContent = typeof userLevel === 'number' ? `Level ${userLevel} Trader` : userLevel;
        }
