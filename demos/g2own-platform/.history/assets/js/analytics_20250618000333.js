...
function initializeAnalytics() {
    ...
    if (consentData && consentData.granted === true) {
        ...
        console.log('G2Own Analytics: No valid consent found, showing banner');
        showConsentBanner();
    }
}
...
function setConsentWithExpiration(granted) {
    ...
    try {
        ...
        document.cookie = `${consentKey}=${JSON.stringify(consentData)};expires=${expires.toUTCString()};path=/;SameSite=Lax`;
        
        console.log('Consent saved:', granted ? 'GRANTED' : 'DECLINED', 'until', expires.toLocaleDateString());
    } catch (e) {
