...
class G2OwnFeaturedCarousel {
    ...
    extractGameContent(gameElement) {
      ...
      setupEventListeners() {
        ...
        // Scroll-based dynamic positioning
        window.addEventListener('scroll', () => {
            this.updateCarouselPositioning();
        });
          console.log('🎯 Event listeners set up including responsive width adjustments and scroll positioning');
    }
    ...
    updateCarouselPositioning() {
        ...
        if (independentCarousel && heroSection && window.innerWidth >= 1024) {
            ...
            // Keep carousel within hero section bounds while scrolling
            let newTop = navbarHeight + 100;
            
            if (scrollY > heroTop - navbarHeight) {
                // Carousel should move with scroll but stay within hero bounds
                const maxTop = heroBottom - scrollY - 400; // 400px is approx carousel height
                newTop = Math.max(navbarHeight + 20, Math.min(navbarHeight + 100, maxTop));
            }
              independentCarousel.style.top = `${newTop}px`;
        }
    }
