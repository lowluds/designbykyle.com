...
class G2OwnAPI {    constructor() {
    ...
    getStoredToken() {
        return localStorage.getItem('ic_token') || sessionStorage.getItem('ic_token');
    }
    ...
    setToken(token, remember = true) {
        ...
        if (remember) {
            ...
            sessionStorage.setItem('ic_token', token);
        }
    }
    ...
    dispatchAuthEvent(type, data = null) {
        window.dispatchEvent(new CustomEvent(`g2own:auth:${type}`, {
            detail: data
        }));
    }
