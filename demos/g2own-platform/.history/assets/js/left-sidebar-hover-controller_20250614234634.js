...
class EnhancedLeftSidebarController extends LeftSidebarController {
    ...
    initHoverSystem() {
        ...
        // Check if hover is supported (not on touch-only devices)
        ...
        if (this.hoverSupported) {
            ...
            // Hide floating toggle when hover system is active
            if (this.floatingToggle) {
                this.floatingToggle.classList.remove('show-fallback');
            }
        } else {
            // Show floating toggle as fallback for touch devices
            console.log('Touch device detected, using floating toggle fallback');
            if (this.floatingToggle) {
                this.floatingToggle.classList.add('show-fallback');
            }
        }
    }
    ...
    adjustForTouch() {
        if (this.hoverZone) {
            this.hoverZone.style.width = '25px'; // Larger touch area
        }
        ...
        // Reduce hover delays for touch
        document.documentElement.style.setProperty('--hover-delay', '100ms');
        document.documentElement.style.setProperty('--hide-delay', '200ms');
    }
    
    adjustForDesktop() {
        if (this.hoverZone) {
            this.hoverZone.style.width = '20px';
        }
        
        // Hide floating toggle on desktop when hover works
        if (this.floatingToggle && this.hoverSupported) {
            this.floatingToggle.classList.remove('show-fallback');
        }
        
        // Standard delays for desktop
        document.documentElement.style.setProperty('--hover-delay', '300ms');
        document.documentElement.style.setProperty('--hide-delay', '500ms');
    }
