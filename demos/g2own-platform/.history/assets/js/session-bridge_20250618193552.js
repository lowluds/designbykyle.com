...
class SessionBridge {    constructor() {
        ...
        const storage = window.safeStorage || sessionStorage;
        const lastRedirect = storage.getItem('last_login_redirect');
        const now = Date.now();
        ...
        // Check window.blockLoginRedirect if it exists (from redirect-lock.js)
        if (window.blockLoginRedirect && window.blockLoginRedirect()) {
            console.warn('⚠️ Login redirect blocked by redirect-lock.js');
            return;
        }
        
        // IMPORTANT: Check if user is already authenticated on community site
        // If they are, we'll import their session instead of redirecting
        try {
            const isAlreadyAuthenticated = await this.checkLoginStateBeforeRedirect();
            if (isAlreadyAuthenticated) {
                console.log('✅ User already authenticated on community, skipping redirect');
                
                // Show success notification using any available notification system
                if (window.loginRedirectHandler && typeof window.loginRedirectHandler.showLoginSuccessNotification === 'function') {
                    window.loginRedirectHandler.showLoginSuccessNotification(this.currentUser);
                }
        ...
        storage.setItem('last_login_redirect', now.toString());
        storage.setItem('g2own_login_initiated', now.toString());
        
        // Also set in localStorage for compatibility with redirect-lock.js
        if (window.safeStorage) {
            window.safeStorage.setItem('login_redirect_time', now.toString());
        } else {
            try {
                localStorage.setItem('login_redirect_time', now.toString());
            } catch(e) {
                console.warn('⚠️ Could not write to localStorage, using fallback');
                if (window._memoryStorage) {
                    window._memoryStorage['login_redirect_time'] = now.toString();
                }
            }
        }
        ...
        if (window.securityUtils && !window.securityUtils.isValidReturnUrl(currentUrl)) {
            ...
            console.log('📍 Redirecting to:', `${this.baseURL}/login/?return=${returnUrl}&_=${timestamp}&no_auto_redirect=1`);
            window.location.href = `${this.baseURL}/login/?return=${returnUrl}&_=${timestamp}&no_auto_redirect=1`;
            return;
        }
    ...
    setupPeriodicSync() {
        ...
        setInterval(() => {
            ...
            // Also use the enhanced session sync if available
            if (window.syncCommunitySession) {
                console.log('🔄 Using enhanced session sync in periodic check');
