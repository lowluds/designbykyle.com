...
function initializeGA4() {
    ...
    if (GA4_CONFIG.measurementId === 'G-XXXXXXXXXX') {        console.log('GA4: Demo mode - analytics disabled for portfolio');
        return;
    }
    
    // Demo mode - no real analytics
    console.log('Demo mode: Google Analytics disabled for portfolio demonstration');
    return;
...
const G2ownAnalytics = {
    ...
    trackNavigation: function(destination, source = 'homepage') {
        ...
        if (destination.includes('example.com/community')) {
...
function showConsentBanner() {
    ...
    banner.innerHTML = `
        ...
        <p>We use analytics to improve your experience on G2Own. Your choice will be remembered for <strong>1 year</strong>.</p>
        <div class="consent-buttons">
            <button id="consent-accept-btn">✓ Accept</button>
            <button id="consent-decline-btn">✗ Decline</button>
        </div>
        <p>You can change this preference anytime in your browser settings.</p>
    `;
    ...
    // Add show class for CSS animation after a brief delay
    setTimeout(() => {
...
function debugConsentStatus() {
    ...
    if (consentData) {
        ...
        // Also check cookie backup
        const cookieMatch = document.cookie.match(new RegExp('(^| )g2own_analytics_consent=([^;]+)'));
        console.log('🍪 Cookie backup:', cookieMatch ? 'Present' : 'Missing');
        ...
        const oldData = localStorage.getItem('g2own_analytics_consent');
        if (oldData) {
            console.log('🍪 Found expired/invalid data:', oldData);
        }
...
function clearAnalyticsConsent() {
    ...
    // Also remove any existing banner
    const banner = document.getElementById('analytics-consent-banner');
    if (banner) {
        banner.remove();
        console.log('🗑️ Removed existing consent banner');
    }
}

// Test function to force show banner (for debugging)
function forceShowConsentBanner() {
