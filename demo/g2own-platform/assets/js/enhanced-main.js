...
class G2OwnEnhanced {
    ...
    setupAuthEventListeners() {
        ...
        window.addEventListener('g2own:auth-update', (event) => {
            this.handleAuthUpdate(event.detail.user);
        });
        ...
        window.addEventListener('message', (event) => {
            // Demo mode - no cross-domain messaging
            console.log('Demo mode: Cross-domain messaging disabled');
            return;

            switch (event.data.type) {
                case 'AUTH_UPDATE':
                    this.handleAuthUpdate(event.data.user);
                    break;
                case 'LOGOUT':
                    this.handleLogout();
                    break;
                case 'CART_UPDATE':
                    this.handleCartUpdate(event.data.cart);
                    break;
            }
        });
    }
    ...
    handleAuthUpdate(user) {
        ...
        // Update UI components for authenticated state
    ...
    handleLogout() {
        ...
        // Update UI components for logged out state
    ...
    handleCartUpdate(cartData) {
        this.state.cartItems = cartData.items || [];
        this.updateCartUI();
    }
    ...
    updatePersonalizedContent(user) {
        ...
        // Update user-specific recommendations if any
        this.loadUserRecommendations(user);
    }

    async loadUserRecommendations(user) {
        // This would load user-specific content from the backend
        try {
            if (this.api && user.id) {
                // Example: Load user's purchase history or recommendations
                // const recommendations = await this.api.getUserRecommendations(user.id);
    ...
    initializeLeftSidebar() {
        // Wait for LeftSidebarController to be available
        if (typeof LeftSidebarController !== 'undefined') {
            this.leftSidebarController = new LeftSidebarController();
            ...
            setTimeout(() => {
                if (typeof LeftSidebarController !== 'undefined') {
                    this.leftSidebarController = new LeftSidebarController();
                    console.log('✅ Left Sidebar Controller initialized (delayed)');
                } else {
                    console.warn('⚠️ LeftSidebarController not available');
                }
            }, 500);
        }
    }
    ...
    onWindowLoad() {
        ...
        this.initializeCategoryCardsObserver(); // Enhanced category cards loading
    ...
    animateLoadingProgress(progressBar, progressText) {
        ...
        if (hero) {
            // Exclude hero-subtitle from JS animations since it has its own CSS animation
            const heroElements = hero.querySelectorAll('h1, .btn, .trust-indicators');
    ...
    // ============ CART SIDEBAR SYSTEM ============    initializeCartSidebar() {
    ...
    updateCartQuantity(productId, newQuantity) {
        ...
        if (cartCount) {
            ...
            if (totalItems > 0) {
                ...
                // Hide cart count with animation
    ...
    animateProgressBar(progressBar) {        const targetWidth = progressBar.dataset.progress || '100';
    ...
    initializeIntersectionObserver() {
        // More aggressive observer for immediate loading
        const observerOptions = {
            threshold: [0, 0.1, 0.25], // Multiple thresholds for better triggering
            rootMargin: '200px 0px 100px 0px' // Start animation much earlier
        };
...
// ============ ENHANCED CATEGORY CARDS LOADER ============
    initializeCategoryCardsObserver() {
        // Special observer for category cards with more aggressive settings
        const categoryObserverOptions = {
            threshold: [0, 0.05, 0.1],
            rootMargin: '300px 0px 200px 0px' // Very generous margins for early loading
        };
        ...
        // Find all category cards and set initial state
...
// Initialize authentication components when DOM is ready
document.addEventListener('DOMContentLoaded', function() {
    console.log('🔧 Initializing G2Own authentication system...');
    
    // Initialize Security Utils first (core security)
    if (typeof SecurityUtils !== 'undefined') {
        window.securityUtils = new SecurityUtils();
        console.log('✅ Security Utils initialized');
    } else {
        console.warn('⚠️ SecurityUtils class not found');
    }
    
    // Initialize Session Bridge (core authentication)
    if (typeof SessionBridge !== 'undefined') {
        window.sessionBridge = new SessionBridge();
        console.log('✅ Session Bridge initialized');
    } else {
        console.warn('⚠️ SessionBridge class not found');
    }
    
    // Initialize Top Navigation Auth
    if (typeof TopNavAuth !== 'undefined') {
        window.topNavAuth = new TopNavAuth();
        console.log('✅ Top Navigation Auth initialized');
    } else {
        console.warn('⚠️ TopNavAuth class not found');
    }
    ...
    if (typeof LeftSidebarController !== 'undefined') {
        ...
        console.warn('⚠️ LeftSidebarController class not found');
    }
    
    // Initialize Login Redirect Handler
    if (typeof LoginRedirectHandler !== 'undefined') {
        window.loginRedirectHandler = new LoginRedirectHandler();
        console.log('✅ Login Redirect Handler initialized');
    } else {
        console.warn('⚠️ LoginRedirectHandler class not found');
    }
    
    // Initialize main enhanced system
    if (typeof G2OwnEnhanced !== 'undefined') {
        window.g2ownEnhanced = new G2OwnEnhanced();
        console.log('✅ G2Own Enhanced system initialized');
    } else {
        console.warn('⚠️ G2OwnEnhanced class not found');
    }
    
    console.log('🎉 G2Own authentication system initialization complete!');
});

// Additional initialization when window loads (for dependencies)
window.addEventListener('load', function() {
    // Ensure all auth components are properly connected
    setTimeout(() => {
        if (window.sessionBridge && window.topNavAuth && window.leftSidebarController) {
            // Trigger initial auth check
            window.sessionBridge.checkSession();
            console.log('🔄 Triggered initial authentication check');
        }
    }, 1000);
});
